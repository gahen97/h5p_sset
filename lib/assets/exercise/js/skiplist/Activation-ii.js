...
var defUpdate = updateActive;

var activeFuncs = {
	"searchPath": defActive,
	"addRemove": getActiveII
};
var updateFuncs = {
	"searchPath": defUpdate,
	"addRemove": updateActiveII
};

getActive = function (){
	return mode.run (activeFuncs);
}

updateActive = function (a) {
	return mode.run (updateFuncs, undefined, a);
}

// options for activating the trailing, leading, new nodes in updateActiveII
const NXT_ACTIVE_OPTIONS = {
	DISABLE: {ptr: CODES.DISABLE, next: CODES.DISABLE, detachable: CODES.DISABLE},
	ENABLE: {ptr: CODES.ENABLE, next: [CODES.DISABLE, CODES.DOM_ACTIVE], detachable: CODES.DISABLE}
}
const NEW_ACTIVE_OPTIONS = {
	DISABLE: {},
	ENABLE: {ptr: CODES.ENABLE, next: CODES.ENABLE, detachable: CODES.ENABLE}
}
const LEAD_ACTIVE_OPTIONS = {
	DISABLE: {},
	ENABLE: {ptr: CODES.DISABLE, next: CODES.ENABLE, detachable: CODES.ENABLE}
}
...
function getActiveII () {
	if (op.get() === "remove") return getActiveRemovals ();
...
// get active for removed ...
// should be leading, leading->next, leading->next->next
function getActiveRemovals (){
	var next = getConnectedNodes (leadingNodes);

	var trail = getConnectedNodes (next);
	return {
		next: trail,
		newNodes: next,
		leading: leadingNodes
